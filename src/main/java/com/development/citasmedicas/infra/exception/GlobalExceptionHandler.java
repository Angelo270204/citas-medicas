...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorData> badCredentials(BadCredentialsException ex) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(new ErrorData("Credenciales inválidas"));
    }

    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<ErrorData> illegalArgument(IllegalArgumentException ex) {
        return ResponseEntity.badRequest().body(new ErrorData(ex.getMessage()));
    }
